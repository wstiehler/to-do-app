...
const useProductInactivateHandler = () => {
  const { token } = useAuth();

  const handleProductInactivate = useCallback(async (todoList) => {
  ...
  return handleProductInactivate;
};

export default useProductInactivateHandler;
